...
export default function RoofTakeoff({ importedDims, onTotalChange }) {
  const [settings, setSettings] = useState({ rafterSpacing: 24, rafterSize: "2x8", pitch: "6/12", wastePercent: 10, sheathingWaste: 8 });
  const [sections, setSections] = useState([{ id: 1, name: "Main Roof", ridgeLength: 40, span: 28 }]);
  ...
  return (
    <div>
      <Section title="Settings" color={colors.blue}>
        <Row>
          <SelectInput label="Rafter Size" value={settings.rafterSize} onChange={(v) => updateSetting("rafterSize", v)} options={[{ value: "2x8", label: "2×8" }, { value: "2x10", label: "2×10" }, { value: "2x12", label: "2×12" }]} />
          <SelectInput label="Spacing" value={String(settings.rafterSpacing)} onChange={(v) => updateSetting("rafterSpacing", +v)} options={[{ value: "16", label: '16"' }, { value: "24", label: '24"' }]} />
          <SelectInput label="Pitch" value={settings.pitch} onChange={(v) => updateSetting("pitch", v)} options={Object.keys(PITCH_FACTORS).map((p) => ({ value: p, label: p }))} />
          <NumberInput label="Waste" value={settings.wastePercent} onChange={(v) => updateSetting("wastePercent", v)} unit="%" />
        </Row>
      </Section>

      <Section title="Roof Sections" color={colors.accent}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, minWidth: 580 }}>
            <thead><tr>{["Section", "Ridge", "Span", "Rftr Len", "Rftrs", "+W", "Sheath", "Mat $", "Labor $", "Total $", ""].map((h) => <th key={h} style={tableHeaderStyle}>{h}</th>)}</tr></thead>
            <tbody>{sections.map((section, i) => {
              const calc = calculations.find((c) => c.id === section.id);
              return (
                <tr key={section.id} style={{ borderBottom: `1px solid ${colors.border}`, background: i % 2 === 0 ? colors.surface : "transparent" }}>
                  <td style={{ padding: 3 }}><input value={section.name} onChange={(e) => updateSection(section.id, "name", e.target.value)} placeholder="Section..." aria-label="Section name" style={{ ...tableInputStyle, width: "100%", minWidth: 65 }} /></td>
                  <td style={{ padding: 3 }}><input type="number" value={section.ridgeLength} onChange={(e) => updateSection(section.id, "ridgeLength", +e.target.value)} aria-label="Ridge length (ft)" style={{ ...tableInputStyle, width: 48, textAlign: "center" }} /></td>
                  <td style={{ padding: 3 }}><input type="number" value={section.span} onChange={(e) => updateSection(section.id, "span", +e.target.value)} aria-label="Span (ft)" style={{ ...tableInputStyle, width: 48, textAlign: "center" }} /></td>
                  <td style={{ padding: "3px 5px", textAlign: "center", color: colors.text, fontFamily: fonts.mono }}>{calc ? `${calc.rafterLength.toFixed(1)}'` : "—"}</td>
                  <td style={{ padding: "3px 5px", textAlign: "center", color: colors.text, fontFamily: fonts.mono }}>{calc?.totalRafters ?? "—"}</td>
                  <td style={{ padding: "3px 5px", textAlign: "center", color: colors.green, fontFamily: fonts.mono, fontWeight: 700 }}>{calc?.raftersWithWaste ?? "—"}</td>
                  ...
                  <td style={{ padding: 3 }}><button onClick={() => removeSection(section.id)} aria-label={`Remove ${section.name || "section"}`} style={deleteButtonStyle}>×</button></td>
                </tr>
              );
            })}</tbody>
          </table>
        </div>
        <button onClick={addSection} style={addButtonStyle}>+ Add Section</button>
      </Section>

      <Section title="Totals" color={colors.green}>
        <Row>
          <ResultCard label="Roof Area" value={Math.round(totals.roofArea)} unit="SF" color={colors.blue} large />
          <ResultCard label="Rafters" value={totals.rafters} unit="pcs" color={colors.accent} large />
